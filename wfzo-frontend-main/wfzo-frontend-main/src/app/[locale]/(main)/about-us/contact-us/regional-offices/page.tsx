...
import transformRegionalOfficeMainPage from '@/lib/utils/transformRegionalOfficeMainPage';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const urlSlug = '/regional-offices';
...
export default async function Index({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_STRAPI_API_BASE_URL}/api/pages?filters[slug][$eq]=regional-offices&populate=*
...
&populate[contents][on][home.featured-member][populate]=*`,
    { next: { revalidate: 21600, tags: ['/api/regional-offices'] } }
  );
...
&locale=en`;
  ...
  const sections = transformRegionalOfficeMainPage(json, allOffices) || {};
  ...
  return (
    <div>
      ...
      <div>
        ...
        <div>
          {sections.regionalOfficeCards &&
            sections.regionalOfficeCards.map((regionalSection: any, index: any) => (
              <GridSection
                key={index}
                heading={regionalSection.title}
                members={regionalSection.offices}
          ...
          {sections.nationalContactPointCards &&
            sections.featuredMember &&
            sections.nationalContactPointCards.map((card: any, idx: any) => (
              <AdvancedCarousel
                key={`ncpCard-${idx}`}
                itemsCount={card.offices.length}
                title={sections.featuredMember?.title || card.title || 'Regional Offices'}
                description={
                  sections.featuredMember?.description || card.description || 'Description'
                }
                pageHeading={false}
                showExploreAll={true}
                exploreAllHref="/about-us/contact-us/national-contact-points"
                visibleSlides={{
                  xs: 1,
                  sm: 2,
                  md: 2,
