...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
    ...
    const urlSlug = '/welcome-from-our-ceo';
...
export default async function Index({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const urlSlug = '/welcome-from-our-ceo';
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_API_BASE_URL}/api/pages?filters[slug][$eq]=welcome-from-our-ceo
...
&populate[seo][populate]=*`,
      { next: { revalidate: 21600, tags: ['/api/welcome-from-our-ceo'] } }
    );
    if (!res.ok) {
